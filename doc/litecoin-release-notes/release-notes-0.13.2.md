...
for previous versions of Gandercoin Core was to raise the default minimum
relay fee.

Gandercoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
transactions for mined blocks. Gandercoin Core will relay transactions with
...
entirely in Gandercoin Core 0.13, and it is at this time undecided whether the
...
Gandercoin Core automatically creates a hidden service to listen on, without
manual configuration. Gandercoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Gandercoin Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Gandercoin
Core to use floating fees. By default, Gandercoin Core will use floating
fees.
...
At all times, Gandercoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Gandercoin Core will never create transactions paying less than
...
`gandercoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
- Adding `dbcache=100` in gandercoin.conf
...
For example:
    ...
    $ src/gandercoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `gandercoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `gandercoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
HD wallets are incompatible with older versions of Gandercoin Core.
...
The two phases are distinct in the Gandercoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Gandercoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Gandercoin.
...
segwit's start date of 1 January 2017 miners can update the Gandercoin
client to Gandercoin Core 0.13.2 to signal for segwit support. When a
...
- `gandercoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `gandercoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
